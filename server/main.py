...
class TokenVerifier:
    ...
    def _parse_token(self, token: str) -> Dict[str, Any]:
        ...
        """
        # Accept tokens generated by our auth server (access_token_XXXXXXXX)
        if token.startswith("access_token_"):
            self.logger.info(f"Validating access token: {token[:30]}...")
            
            # Consultar el auth server para obtener información del usuario
            try:
                # Llamar al endpoint /oauth/tokeninfo del auth server
                tokeninfo_url = f"{self.auth_settings.issuer_url}/oauth/tokeninfo"
                ...
                with httpx.Client() as client:
                    ...
                    if response.status_code == 200:
                        ...
                        self.logger.info(f"Token válido para usuario: {token_info.get('email')} (Cliente: {token_info.get('client_name')})")
                        
                        # Retornar claims del token con información del usuario
                        return {
                            ...
                            "scope": token_info.get("scope", self.auth_settings.required_scope),
                            "exp": int(datetime.fromisoformat(token_info["expires_at"]).timestamp()),
                            "iat": int(datetime.now(timezone.utc).timestamp())
                        }
                    else:
                        self.logger.warning(f"Token inválido o expirado según auth server")
                        raise ValueError("Token inválido")
                        
            except Exception as e:
                self.logger.error(f"Error al validar token con auth server: {str(e)}")
                # Fallback a validación mock básica en caso de error
                self.logger.warning("Usando validación mock como fallback")
                return {
                    "iss": self.auth_settings.issuer_url,
                    "aud": self.auth_settings.audience,
                    "sub": "fallback_user",
